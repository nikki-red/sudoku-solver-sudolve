...
function readInput() {
  ...
  for (var i = 0; i < 81; i++) {
    ...
    if (val) { // If cell if filled by user
  ...
  if (!checkInput(grid, x1, y1, x2, y2)) {
    ...
    document.querySelectorAll('input')[parseInt(x1 * 9) + parseInt(y1)].classList.add('invalidCell');
    document.querySelectorAll('input')[parseInt(x2 * 9) + parseInt(y2)].classList.add('invalidCell');
